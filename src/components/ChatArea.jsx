...
//       <button className="mt-8 ml-2" onClick={() => navigate('/friends')}>
//         <ArrowBackIcon />
//       </button>
          ...
          {messages.length > 0 ? (
            <AnimatePresence>
              {messages.map((msg) => (
                ...
                >
                  <div className="max-w-64 mx-4 mt-2 bg-purple-100 p-2 rounded-xl break-words overflow-auto" style={{ maxHeight: '300px' }}>
                    <div className="font-bold text-pink-500">{msg.sender.username}</div>
                    {msg.attachment?.length > 0 ? (
                      msg.attachment.map((att) => {
                        if (!att.url) {
