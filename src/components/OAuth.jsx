...
function OAuth() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:4000/user/oauth', data, { withCredentials: true });
      console.log("My data from server", response.data.data.accessToken);
  ...
  return (
    <div>
      <button onClick={handleSubmit} type="button" className={"rounded flex justify-around items-center google-btn bg-slate-100 p-1 mb-3 h-8  w-72 font-semibold"}>
        <img
